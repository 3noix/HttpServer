	...
	sslConfig{sslConfig}
{
	...
	this->createSocket(socketDescriptor);

	// Connect signals
	QObject::connect(socket,       &QTcpSocket::readyRead,    this, &HttpConnection::read);
	QObject::connect(socket,       &QTcpSocket::bytesWritten, this, &HttpConnection::bytesWritten);
	QObject::connect(socket,       &QTcpSocket::disconnected, this, &HttpConnection::socketDisconnected);
	QObject::connect(timeoutTimer, &QTimer::timeout,          this, &HttpConnection::timeout);
}
...
void HttpConnection::bytesWritten(qint64 bytes)
{
	...
	while (!pendingResponses.empty())
	{
		...
		if (!response->isSending()) {break;}

		// If writeChunk returns false, means buffer is full
		if (!response->writeChunk(socket)) {break;}

		// Read connection header, default to keep-alive
		QString connection;
		if (!response->header("Connection", &connection)) {connection = "keep-alive";}
...
void HttpConnection::timeout()
{
	...
	if (!currentResponse) {currentResponse = new HttpResponse{config};}
...
void HttpConnection::sslErrors(const QList<QSslError> &errors)
{
	if (config->verbosity >= HttpServerConfig::Verbose::Warning)
	{
		...
		auto addErrorStr = [] (const QString str, const QSslError &error) {return str.isEmpty() ? error.errorString() : str + ", " + error.errorString();};
		QString errorMessages = std::accumulate(errors.begin(), errors.end(), QString{}, addErrorStr);
