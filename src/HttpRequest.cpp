...
HttpRequest::HttpRequest(HttpServerConfig *config) : config{config}
{
}
...
HttpRequest::~HttpRequest()
{
	...
	for (auto kv : formFiles_) {delete kv.second.file;}
	formFiles_.clear();
	...
	while (true)
	{
		switch (state_)
		{
			case State::ReadRequestLine:
				if (!parseRequestLine(socket, response)) {return false;}
				break;

			case State::ReadHeader:
				if (!parseHeader(socket, response)) {return false;}
				break;

			case State::ReadBody:
				if (!parseBody(socket, response)) {return false;}
				break;

			case State::ReadMultiFormBodyData:
			case State::ReadMultiFormBodyHeaders:
				if (!parseMultiFormBody(socket, response)) {return false;}
				break;
...
bool HttpRequest::parseMultiFormBody(QTcpSocket *socket, HttpResponse *response)
{
	...
	else if (state_ == State::ReadMultiFormBodyHeaders)
	{
		...
		int index = buffer.indexOf("\r\n\r\n");
		if (index == -1) {return !chunk.isEmpty();}
...
QString HttpRequest::formFile(QString key) const
{
	...
	return (it != formFields_.end() ? it->second : QString{});
}
...
QString HttpRequest::cookie(QString name) const
{
	...
	return (it == cookies.end() ? QString{} : it->second);
}
